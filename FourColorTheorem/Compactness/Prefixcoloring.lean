...
-- theorem minimalColoring_of_partialEqOn {m : Map} {nc : ℕ} {k0 k1 : Map} {n : ℕ}
--   (k0k1 : partialEqOn m n k0 k1) (min_n2 : minimalColoring m nc k0 n) :
--   minimalColoring m nc k1 n := by{
--     induction n with
--     | zero => simp
--     | succ n' ih => {
--       intro i le_i_n' j k k1k
--     }
--     -- intro i lt_i_n j k k1k h1 h2
--     -- have k0k1' := partialEqOn_of_partialEqOn_of_ge (le_of_lt lt_i_n) k0k1
--     -- have k0k := partialEqOn_trans k0k1' k1k
--     -- have k0k1'' := partialEqOn_of_partialEqOn_of_ge (Nat.succ_le_of_lt lt_i_n) k0k1
--     -- rw[Map.partialMap] at h2
--     -- have ⟨j0, le_j0j, k0j0i⟩:=min_n2 i lt_i_n j k k0k h1 h2
--     -- refine ⟨j0, le_j0j, ?_⟩

--   }

-- theorem minimalColoring_of_le {m : Map} {nc : ℕ} {k0 : Map} {n1 n2 : ℕ}
--   (le_n12 : n1 ≤ n2) (min_n2 : minimalColoring m nc k0 n2) :
--   minimalColoring m nc k0 n1 := by{

--   }

-- theorem prefixColoring_of_le {m : Map} [IsPlainMap m] {nc : ℕ} {k : Map}
--   [IsPlainMap k] {n1 n2 : ℕ}
--   (le_n12 : n1 ≤ n2) (kpre2 : prefixColoring m nc k n2) :
--   prefixColoring m nc (m.partialMap n1 k) n1 := by{
--     unfold prefixColoring
--     apply And.intro partialMap_submap_partialMap_partialMap
--     have ⟨h0, h1, h2⟩:=kpre2
--     constructor
--     · {
--       apply extensible_partialMap_of_le (n2:=n2) le_n12
--       exact h1
--     }
--     intro i lt_in1 j k' k'p k'e k'm
--     have lt_in2 := lt_of_lt_of_le lt_in1 le_n12
--     have kp : partialEqOn m i k k' := by{
--       have h0:=partialEqOn_partialMap (m:=m) (n:=n1) (k:=k)
--       have h1:=partialEqOn_of_partialEqOn_of_ge (le_of_lt lt_in1) h0
--       exact partialEqOn_trans h1 k'p
--     }
--     have h2':=h2 i lt_in2 j k' kp k'e k'm
--     have ⟨j0, le_j0j, kj0i⟩:=h2'
--     rw[Map.partialMap] at k'm
--     have in1 : stdPoint i ∈ m.partialRegion n1 := by{
--       apply partialRegion_subset_of_le (Nat.succ_le_of_lt lt_in1)
--       apply k'm.2.2
--     }
--     simp only [Map.partialMap, in1, and_true]
--     rcases lt_or_ge j i with lt_ji | le_ij
--     · {
--       refine ⟨j, le_refl _, ?_,
--       partialRegion_subset_of_le (Nat.succ_le_of_lt lt_in1) k'm.2.1⟩

--     }
--     · {
--       refine ⟨i, le_ij, ?_, in1⟩
--       apply (refl_of_rel kj0i).right
--     }
--   }

-- theorem prefixColoring_unique {m : Map} [IsSimpleMap m] {nc : ℕ} {k1 k2 : Map} {n1 n2 : ℕ}
--   (k1pre : prefixColoring m nc k1 n1) (k2pre : prefixColoring m nc k2 n2)
--   : partialEqOn m (min n1 n2) k1 k2 := by{
--     wlog le_n12 : n1 ≤ n2 with H
--     · {
--       have le_n21 := le_of_not_ge le_n12
--       rw[min_comm]
...
--       assumption
--     }
--     wlog eq_n12 : n1 = n2 with H
--     · {
--       have lt_n12 := lt_of_le_of_ne le_n12 eq_n12
