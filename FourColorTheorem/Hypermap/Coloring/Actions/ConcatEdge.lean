...
theorem fourColorable_of_concatEdge_fourColorable
  (Hp : H.Plain) (Hb : H.Bridgeless)
  {x : α} (hxv : H.isSubdivVertice x)
  (H2c' : (H.concatEdge hxv).fourColorable) : H.fourColorable := by{
  have ⟨k2, hk2f, hk2e⟩:=H2c'
  have hcf'' := isColoring.cface_invariant ⟨hk2f, hk2e⟩
  let k : α → FourColor := fun y =>
    if hz : ∃z : {a : {a // a ≠ x} // a ≠ ⟨node x, hxv.node_ne⟩}, H.cface y z then
  ...
  have hf_iff : face x = x ↔ face (node x) = node x := by{
    ...
    · {
      ...
      rw[hxv.node_2, nfe_cancel]
    }
    · {
      intro hfnx
      have h0 := H.nfe_cancel (H.edge (H.node x))
      rw[(Hp (node x)).left, hfnx, hxv.node_2] at h0
  ...
  · {
    ...
    · {
      have hna : node (node a) = a := by{
        rcases ha with ha | ha <;> simp[ha, hxv.node_2]
      }
      have hfna : face (node a) ≠ a := by{
        rcases ha with ha | ha <;> simpa[ha, hxv.node_2]
      }
      unfold k
      ...
      · {
        ...
        have hz0 : ¬∃ z : {a : {a // a ≠ x} // a ≠ ⟨node x, hxv.node_ne⟩}, H.cface a z := by{
          intro ⟨z, hz⟩
        ...
        have hz1 : ¬∃ z : {a : {a // a ≠ x} // a ≠ ⟨node x, hxv.node_ne⟩}, H.cface (edge a) z := by{
          intro ⟨z, hz⟩
          rw[cface, funReflTransGen_iff_iterate] at hz
          rcases hz with ⟨n, hn⟩
          ...
          rcases ha with ha | ha
          · have hz := z.prop; simp[Subtype.ext_iff,  ← hn, ha] at hz
          · have hz := z.val.prop; simp[← hn, ha, hxv.node_2] at hz
        }
        have hfx : face x = x :=by{
          rcases ha with ha | ha
          · exact ha ▸ hfa
          · rw[ha, hxv.node_2] at hfa''; exact hfa''
        }
        rw[dif_neg hz0, dif_neg hz1, FourColor.ofBits_injective.ne_iff]
        simp only [ne_eq, beq_eq_beq]
        rcases ha with ha | ha
        · simp[ha, Hp.edge_ne]
        · simp only [ha, hxv.node_ne, iff_false, not_not]; rw[← H.face_inj, fen_cancel, hfx]
      }
      have hfx' : face x ≠ x ∧ face (node x) ≠ node x := by{
        rcases ha with ha | ha <;> simp only [ha] at hfa
        · apply And.intro hfa
          rw[← H.node_injective.ne_iff, hxv.node_2, ← edgeinv_apply]
      ...
      let a' : {a : {a // a ≠ x} // a ≠ ⟨node x, hxv.node_ne⟩} :=
        ⟨⟨H.faceinv a, hfa'''.1⟩, by{simp[hfa'''.2]}⟩
      let ea' : {a : {a // a ≠ x} // a ≠ ⟨node x, hxv.node_ne⟩} :=
        ⟨⟨edge a, hfa''.1⟩, by{simp[hfa''.2]}⟩
      have hz0 : ∃z : {a : {a // a ≠ x} // a ≠ ⟨node x, hxv.node_ne⟩}, H.cface a z := by{
        use a'
        apply H.cface_equivalence.symm
        apply ReflTransGen.single
      ...
      rw[← hcf''0, ← hcf''1]
      have ih : ea' = ((H.WalkupE x).WalkupE ⟨_, hxv.node_ne⟩).edge a' := by{
        unfold ea' a'
        simp only [faceinv_apply]
        rcases ha with ha | ha
        · simp only [ha]
          rw[concatedge_edge_val_edge_node_of_plain_of_subdiv' Hp hxv rfl]
        · simp only [ha, hxv.node_2]
          rw[concatedge_edge_val_edge_of_plain_of_subdiv' Hp hxv rfl]
      }
      rw[ih]
      apply hk2f
    }
    ...
    · {
      have hfx' : face x ≠ x ∧ face (node x) ≠ node x := by{
        ...
        rw[← H.node_inj, ← H.edge_inj, enf_cancel, hxv.node_2, ← hea] at hfx''
        exact ha.2 hfx''.symm
      }
      ...
      have ih : ea' = ((H.WalkupE x).WalkupE ⟨_, hxv.node_ne⟩).edge a' := by{
        ...
        · simp only [hea, Hp.edge_edge]
          rw[concatedge_edge_val_edge_of_plain_of_subdiv' Hp hxv rfl]
        · simp only [hea, Hp.edge_edge, hxv.node_2]
          rw[concatedge_edge_val_edge_node_of_plain_of_subdiv' Hp hxv rfl]
      }
      rw[ih]
      apply hk2f
    ...
    have h0 :=
      ...
      hea.1 hea.2
    simp only at h0
    have hz1 : ∃ z : {a : {a // a ≠ x} // a ≠ ⟨node x, hxv.node_ne⟩}, H.cface (edge a) ↑↑z :=
      ⟨((H.WalkupE x).WalkupE ⟨_, hxv.node_ne⟩).edge ⟨⟨a, ha.1⟩, by{simp[ha.2]}⟩,
        by{simp[h0, cface, funReflTransGen]; rfl}⟩
    rw[dif_pos hz0, dif_pos hz1]
    have hz0_spec := Classical.choose_spec hz0
    have hz1_spec := Classical.choose_spec hz1
    have hcf''0 := hcf'' ⟨⟨a, ha.1⟩, by{simp[ha.2]}⟩ (Classical.choose hz0)
      (by{simpa[walkupe_cface]})
    have hcf''1 := hcf'' (((H.WalkupE x).WalkupE ⟨_, hxv.node_ne⟩).edge ⟨⟨a, ha.1⟩, by{simp[ha.2]}⟩)
      (Classical.choose hz1) (by{simpa[walkupe_cface, h0]})
    rw[← hcf''0, ← hcf''1]
    apply hk2f
  }
  · exact hcf
