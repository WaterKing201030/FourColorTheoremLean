...
def ext1Disk (m : Matte) (d : GDart) : List GPixel := d.half :: m.disk
...
theorem mem_ext1Ring_iff {m : Matte} {d : GDart} (hc : ext1Hp m d) :
  ∀x, x ∈ ext1Ring m d ↔ x ∈ border (ext1Disk m d) := by{
    ...
    rw[ext1Disk, border, Set.mem_setOf, List.mem_cons, List.mem_cons, not_or]
    constructor
    · {
      intro h
      cases h with
...
theorem mem_ext2Ring_iff {m : Matte} {d : GDart} (h : ext2Hp m d)
  : ∀x, x ∈ ext2Ring m d ↔ x ∈ border (ext2Disk m d) := by{
    ...
    rw[ext2Disk, border, Set.mem_setOf, List.mem_cons, List.mem_cons, not_or]
    constructor
    · {
      intro h'
      cases h' with
