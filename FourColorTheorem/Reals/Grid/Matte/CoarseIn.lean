  ...
  : extendsIn m r p := by{
    ...
    | n + 1 => {
      ...
      | ind n IHn => {
        ...
        have IHp {d : GDart} (Dp : d.half = p) (m_r0d : ∃q ∈ r0.toRegion \ chop1 d, q ∈ m) :
          ...
          · {
            apply extendsIn_ehex r_p (d:=d) Dp (by{
              ...
              refine ⟨a, ?_, h0⟩
              rw[←Dp] at r01p
              rw[mem_chop1Rect_inner_iff_mem_inner_chopRect] at r01p
              rw[GRectangle.mem_iff_toRegion, chopRect_toRegion, Set.mem_inter_iff] at r01p
              rw[←GRectangle.mem_iff_toRegion, GRectangle.mem_inner_iff] at r01p
              rw[GRectangle.mem_iff_toRegion, chop1Rect_toRegion, Set.mem_inter_iff]
              rw[←GRectangle.mem_iff_toRegion]
              constructor
              · apply r01p.left; apply ehex_subset_touch; apply GRectangle.mem_enum_iff.mp h1
              · apply chop_subset_chop1; apply chopRect_subset_chop (r:=d.half.touch); rw[←ehex]
                rw[←GRectangle.mem_iff_toRegion, ←GRectangle.mem_enum_iff]; exact h1
            })
        ...
        let nd (q : GPixel) : GDart := 2 • q + q.mod2.ccw.ccw.ccw
        have nd_half {q : GPixel} : (nd q).half = q := by{
          rw[GPoint.half_add_double]
          repeat rw[GPoint.mod2_ccw]
          rw[GPoint.half_mod2, add_zero]
        }
        have nd_mod2 {q : GPixel} : (nd q).mod2 = q.mod2.ccw.ccw.ccw := by{
          rw[GPoint.mod2_add_double]
          repeat rw[GPoint.mod2_ccw]
          rw[GPoint.mod2_mod2]
        }
        have hEnd {q : GPixel} : (edge (nd q)).half = node q := by{
        ...
        have ndN {q : GPixel} : nd (node q) = edge (node (edge (nd q))) := by{
        ...
        have ndFE {q : GPixel} : nd (face (edge q)) = edge (face (nd q)) := by{
          nth_rw 1 [←node_4 (d:=q)]
        ...
        have ehex_shift_quad {q q' : GPixel} :
        ...
        · {
          have ⟨q, hq0, hq1⟩ : ∃q, (∃q' ∈ r0.toRegion \ chop1 q, q' ∈ m)
            ∧ q.half = p
          := by{
            have ⟨q, r_q, m_q⟩:=m_r
            by_contra h
